...
```
```
相對而言，以下哪一種設備或服務，是在ISO/OSI模型中，最低層的東西?
(A)	DNS server  (B)Repeater (C)Bridge  (D)Router    
答案: B
```
```
Devices that amplify signals along a network are called:
(A) concentrators    (B) repeaters  (C) routers    (D) bridges (E) gateways.

答案:B
```
```
In many telecommunications networks, amplifying signals between stations is done by a: 
...
複選題形式
```
________ operate ate the first two layers of the OSI model. 
...
(1)星形架構(star)，(2)環狀架構(ring)，及(3)匯流排架構(bus)，試說明以上三者之異同及優缺點。
```
...
(C)types of networks (D)devices connected to a network    
